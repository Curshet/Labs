...
let meals: [String: Meal] = ["завтрак": Meal(food: ["тост", "апельсиновый сок", "яичные белки"], calories: 530)]


func bypassFunc(input: String) -> Meal? {
    
    let cases: Set = ["завтрак", "обед", "ужин"]
    
    var result: Meal?
    
    cases.contains(input) ? (result = meals[input]) : (result = nil)
    
    return result
    
}

bypassFunc(input: "завтрак")
bypassFunc(input: "полдник")
/*:
 ...
 */
func checkList(dictionary: [String: Any], key: String) -> [String: Any] {
...
checkList(dictionary: meals, key: "завтрак")
